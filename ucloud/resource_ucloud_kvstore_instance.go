...
func createActiveStandbyRedisInstance(d *schema.ResourceData, meta interface{}) error {
	...
	if v, ok := d.GetOk("name"); ok {
		...
		req.Name = ucloud.String(resource.PrefixedUniqueId("tf-kvstore-instance-"))
	}
	...
	parameterGroupId, err := setKvstoreDefaultParameterGroup(d, client)
	if err != nil {
		return err
	} else {
...
func setKvstoreDefaultParameterGroup(d *schema.ResourceData, client *UCloudClient) (string, error) {
	conn := client.pumemconn
	limit := 100
	offset := 0
	...
	for {
		...
		if err != nil {
			return "", fmt.Errorf("error on reading db parameter group when creating, %s", err)
		}

		if resp == nil || len(resp.DataSet) < 1 {
			return "", fmt.Errorf("error on querying defult value of db parameter group")
		}
	...
	return "", fmt.Errorf("can not get the default parameter group")
}
